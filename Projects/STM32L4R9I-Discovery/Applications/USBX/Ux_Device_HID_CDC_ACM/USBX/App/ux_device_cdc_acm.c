...
void CDC_Init_FS(void *cdc_acm)
{
  ...
  /* Get uart3 baudrate */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_baudrate = huart2.Init.BaudRate;
  ...
  /* Get uart3 Parity */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_parity = huart2.Init.Parity;

  /* Get uart3 StopBits */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_stop_bit = huart2.Init.StopBits;
    ...
    if (device->ux_slave_device_state == UX_DEVICE_CONFIGURED)
    {
      ...
      if (ux_status == UX_SUCCESS)
      {
        ...
        if (actual_length != 0)
        {
          ...
          if (HAL_UART_Transmit_DMA(&huart2, (uint8_t *)UserRxBufferFS,
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (HAL_UART_Receive_IT(&huart2, (uint8_t *)UserTxBufferFS + UserTxBufPtrIn, 1) != HAL_OK)
  {
    /* Transfer error in reception process */
    Error_Handler();
  }
                                ...
                                *CDC_VCP_LineCoding)
{
  ...
  huart2.Init.BaudRate = CDC_VCP_LineCoding->ux_slave_class_cdc_acm_parameter_baudrate;
