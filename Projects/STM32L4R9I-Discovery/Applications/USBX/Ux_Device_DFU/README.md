...
In this application, two operating modes are available:
 1. DFU operating mode:
    This mode is entered after an MCU reset in case:
     - The DFU mode is forced by the user: the user presses the Joy.
 ...
 2. Run-time application mode:
    This is the normal run-time activities. A binary which toggles LED1 and LED2 on the STM32L4R9I-Discovery board "STM32L4R9I_LED_Toggle_@0x8020000" is provided in Binary directory.
...
After each device reset (unplug the STM32L4R9I-Discovery board from PC), Plug the STM32L4R9I-Discovery board with Key JOY pressed to enter the DFU mode.
...
When plugged to PC host, the STM32L4R9 must be properly enumerated as an USB DFU device.
During the enumeration phase, device provides host with the requested descriptors (Device, configuration, string).
Those descriptors are used by host driver to identify the device capabilities.
Once the STM32L4R5ZI USB device successfully completed the enumeration phase, Open DfuSe you can now download and upload to/from media .
...
In order to make the program work, you must do the following :
 ...
 - To run the application, proceed as follows:
   ...
   - For Windows 10 and later : Update STM32 DFU device driver manually from Windows Device Manager.
     The install of required device driver is available under:
     "Program Files\STMicroelectronics\Software\DfuSe v3.0.6\Bin\Driver\Win10" directory.
   - Open "DfuSe Demo", choose the "STM32L4R9I_LED_Toggle_@0x8020000.dfu" provided in Binary
