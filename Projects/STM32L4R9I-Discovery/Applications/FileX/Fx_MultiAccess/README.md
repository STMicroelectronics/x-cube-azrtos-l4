...
This application provides an example of Azure RTOS FileX stack usage on STM32L4R9I-Disco board, it demonstrates the FileX's concurrent file access capabilities. The application is designed to execute file operations on the SD card device, the code provides all required software code for handling SD card I/O operations.
...
When an SD card is inserted into the STM32L4R9I-Disco SD card reader and the board is powered up, the application starts file operations. 
...
- FileX is using data buffers, passed as arguments to fx_media_open(), fx_media_read() and fx_media_write() API it is recommended that these buffers are multiple of sector size.
