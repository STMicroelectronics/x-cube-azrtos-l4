...
VOID fx_thread_entry(ULONG thread_input)
{
  ...
  /* Configure flash erase of bank 2 */
  ...
  do {
    ...
    /* Program 8 bytes into flash */
    flash_ret = HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, (uint32_t) FlashAddress, *((uint64_t*) read_buffer));
    ...
    /* Increment the flash address by 8 bytes */
