...
This application provides an example of Azure RTOS USBX stack usage on STM32L4R5ZI NUCLEO board,
...
When plugged to PC host, the STM32L4R5ZI must be properly enumerated as an USB HID mouse device.
During the enumeration phase, device provides host with the requested descriptors (Device, configuration, string).
Those descriptors are used by host driver to identify the device capabilities.
Once the STM32L4R5ZI USB device successfully completed the enumeration phase, the device sends a HID report after a user button press.
